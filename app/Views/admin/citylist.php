            ...
            <div class="col-lg-12">
                                        ...
                                        <?php if ($data): $i = 1;
                                         ...
                                         ?>
                                            <?php foreach ($data as $key => $value): ?>
                                                ...
                                                <td><?= $value['title'] ?></td>
                                                <td><?php //$value['title'] 
                                                  $state = $this->common_model->GetSingleData('states',array('id'=>$value['state_id']));
                                                  echo $state['title'];
                                                ?></td>
                                                <td>
              ...
              <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                  ...
                  <!-- Modal body -->
                  <form id="edit_city" method="post" action="#" onsubmit="return edit_city(this , <?= $value['id']; ?>)" >
                      ...
                      <div>
                        ...
                        <select name="state" class="form-control" required>
                         ...
                         <?php
                         $state = $this->common_model->GetAllData('states','','id','desc');
                       foreach($state as $sval) {
                        ?>
                           <option <?= ($sval['id'] ==  $value['state_id']) ? "selected":""; ?> value="<?php echo $sval['id']; ?>"><?php echo $sval['title'] ?></option>
                        <?php
                          }
                        ?>
                      ...
                      <div class="mt-3 text-center">
                          ...
                          <div class="remove">
                              <button class="btn btn-danger" id="delete_btns" onclick="delete_city(<?= $value['id'] ?>)">Delete</button>
                          </div>
    ...
    <div class="modal" id="myModal">                 
        <div class="modal-dialog modal-dialog-centered">
          <div class="modal-content">
            ...
            <!-- Modal body -->
            <form id="add_city" method="post" action="#" onsubmit="return add_city()" >
                 <div class="modal-body">
                <div class="col-md-12 py-3">
                    <div>
                        <label>Title</label>
                    ...
                    <div>
                       ...
                       foreach($state as $sval) {
                        ?>
                           <option value="<?php echo $sval['id']; ?>"><?php echo $sval['title'] ?></option>
                        <?php
                          }
                        ?>
...
function add_city() {
  $('.alert-danger').remove();
      ...
      $.ajax({
      url: '<?= base_url() ?>/Admin/City_Management/addCity',
      type: 'POST',
      cache:false,
      contentType: false,
      processData: false,
      data:new FormData($('#add_city')[0]),
      dataType: 'json',
      beforeSend: function() {        
...
function edit_city(el , id) {
    $('.alert-danger').remove();
      $.ajax({
      url: '<?= base_url() ?>/Admin/City_Management/editCity',
      type: 'POST',
      cache:false,
      contentType: false,
      processData: false,
...
function delete_city(id) {
    ...
    {
        $.ajax({
      url: '<?= base_url() ?>/Admin/City_Management/deleteCity',
      type: 'POST',
      cache:false,
      data:{'id':id},
      dataType: 'json',
